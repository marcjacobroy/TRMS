window.onload = function() {
    let btn = document.getElementById("createAwardButton");
...
function makeAward(requestId){
    ...
    const limit = 1000;
    let exceeding = (limit - employee.awardAmount - employee.pendingAmount - value < 0);
    
    const endpoint = "/award";
    const body = "value=" + value + "&justification=" + justification + "&awarded=" + false + "&exceeding=" + exceeding + "&requestId=" + requestId + "&accepted=" + false;
    const verb = "POST";
    const output = false;
...
function makeMessageToDS(message, ds){
    ...
    let subject = "Exceeding available funds email pertaining your direct supervisee " + currEmployee.firstName + " " + currEmployee.lastName + " on request with id " + sessionStorage.getItem("requestId"); 
